...
} from '@/services/api/Model/CreateUserInviteModel';
...
export default class CreateUserDialog extends Vue implements OnCreated {
    ...
    public checkSpn() {
        ...
        validate(this.userModel.psn, 'required|min:10|max:10').then(async (result: ValidationResult) => {
            if (result.valid && this.userModel.psn) {
                try {
                    const result = await UserModule.initCheckPSN(this.userModel.psn);
                    this.userModel.dateBirth = result._payload.dateBirth;
                    this.userModel.firstName = result._payload.firstName;
                    this.userModel.lastName = result._payload.lastName;
                    this.userModel.patronymic = result._payload.patronymic;
    ...
    public createUser(validate: Promise<any>) {
        validate.then(async (result: boolean) => {
            ...
            try {
                const result = await UserModule.createUser(this.userModel);
