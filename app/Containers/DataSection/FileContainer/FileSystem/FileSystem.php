...
final class FileSystem implements FileSystemContract
{
     ...
     * @return array|null
     */
    public function loadPdf(PDF $file): ?array
    {
        return $this->saveGeneratedPdf($file);
    }
     ...
     * @return array|null
     */
    public function saveGeneratedPdf(PDF $file): null|array
    {
        $tmpFile = $this->createFileFromBinary($file->output());

        $uploadedFileInstance = $this->createUploadedFileInstance(
            $tmpFile['tmpPathName'],
            $tmpFile['tmpFileName'] . '.pdf',
            $tmpFile['tmpFileMimeType'],
        );

        $storedFile = $this->signedDocuments()
            ->saveInBucket($uploadedFileInstance);
